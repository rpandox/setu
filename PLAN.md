...
| Forward auto-start hook (Phase 6 kickoff) | **A sessions-store subscription fires `auto` rules when a host's session transitions to running**, deduped against live ruleKeys | Catches every connect path (tab, split, duplicate, restore) exactly once; a per-component hook would miss or double-fire | — |
...
| Snippet pack file dialogs (Phase 6 kickoff) | **`tauri-plugin-dialog` + `@tauri-apps/plugin-dialog` added (pinned)** for native open/save panels | Packs need real file picking; the official plugin is small, and the TOML text still crosses IPC as a string | `<input type=file>` import + clipboard export if the plugin misbehaves |
...
ForwardsPopover, HostEditor (right drawer), Settings (window), Toast, ConfirmDialog,
