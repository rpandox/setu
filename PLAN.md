...
| Sync conflict behavior (Phase 8 kickoff) | **A conflicted rebase is left in progress**: the dot goes `conflict`, the footer offers Open in Finder + the F10 resolution doc, and "Cancel sync" runs `git rebase --abort`. Never auto-resolved | F10's persistent conflict dot implies the conflict state survives; aborting silently would hide the divergence and re-create it every sync. The markers on disk are the user's to resolve (chezmoi/git users) with an explicit one-click escape | — |
