...
| Settings window mechanics (Phase 8 kickoff) | **A real second Tauri WebviewWindow** (§7 says window; user confirmed over an overlay): same Vite bundle, `?window=settings` query-param branch in `main.tsx`, created/focused via a Rust `settings_window_open` command. Separate webview ⇒ separate store instances ⇒ a **`settings:changed` event** (emitted by Rust after every successful save) is the one cross-window propagation channel — the main window reloads and hot-applies from it | First multi-window surface in the app; query-param routing avoids a router dep and keeps tokens/theme shared, and the Rust-side builder gives window config a single home. Hot-apply cannot ride a direct store call because the windows don't share a JS realm | Fold back into an in-app overlay (SftpPanel precedent) if multi-window focus/shortcut handling proves hostile |
| Settings IPC family (Phase 8 kickoff) | **`settings_get` / `settings_set` carry the whole document** (`ui_state_get/set` precedent) with validation errors result-side (`{ settings?, errors: FieldError[] }`, hosts precedent); `settings_set` replaces the launch-time cache (reactivating the Phase 7 escape hatch) and emits `settings:changed`. `settings_window_open`, `git_sync_set_remote`, `snapshot_now`, and `sync_open_dir` join beyond §3's table row | §3's table predates the Settings window; whole-document writes keep the atomic-rename story simple and partial-update races impossible | Per-section commands if the document ever grows contended writers |
| Sync remote source of truth (Phase 8 kickoff) | **The git remote lives in `.git/config` (`origin`), never in settings.toml** — `[sync]` holds only `auto_sync_on_quit` and snapshot knobs. Every git call runs `GIT_TERMINAL_PROMPT=0` + `GIT_SSH_COMMAND="ssh -oBatchMode=yes"` + a 30s cap; commit identity falls back to repo-local `setu`/`setu@<hostname>`; auto-sync-on-quit is capped at 10s inside the exit sweep | settings.toml is itself synced: a remote URL inside it would overwrite each machine's own remote on every pull. Prompt-free, time-capped git keeps Sync now from ever hanging the UI or quit | — |
| Sync conflict behavior (Phase 8 kickoff; precised at the live walk) | **A conflicted rebase is left in progress**: the dot goes `conflict`, the footer offers Open in Finder + the F10 resolution doc, and "Cancel sync" runs `git rebase --abort`. Never auto-resolved. **After the user fixes the markers, the next Sync now finishes the paused rebase** (stage → `rebase --continue` → push); files still containing markers keep refusing — the check reads file contents, because `git add` would clear the unmerged flag no matter what the file says | F10's persistent conflict dot implies the conflict state survives; aborting silently would hide the divergence and re-create it every sync. The live walk showed the documented "fix markers, Sync now again" path needed a real code path — without it a resolved rebase had no in-app way to finish | — |
| Secrets lint placement (Phase 8 kickoff) | **Rust-side, over the working-tree files about to be staged** (vault.rs walk: skip `.git`, symlinks), before `git add`; blocks result-side with `{ file, line_no, line, rule }`. Heuristics: secret-key assignments (`password/passwd/token/secret/api_key/private_key` = non-empty value), PEM private-key headers, base64 runs ≥ 40 chars — with an **ssh-public-key allowlist** (`ssh-ed25519`/`ssh-rsa`/`ecdsa-*` lines pass) | Linting the exact bytes about to be committed (not a git hook) keeps the block portable, testable, and impossible to bypass by hook removal; hosts.toml legitimately carries pubkey base64 that the raw run-length rule would flag | Tune patterns (or add a per-line override) if real configs surface new false positives |
| Snapshot format (Phase 8 kickoff) | **`.tar.gz` via the existing `tar` crate + `flate2`, not F10's literal "zip"** — `state_dir/snapshots/setu-config-<UTC ts>.tar.gz`, schedule derived from the newest archive's mtime (no state file), prune to `keep` (default 10). `flate2`, `gethostname`, and `time` join §11 | Reuses the Phase 7 vault walk + tar dep; a `zip` crate would be a heavier new dependency for the same double-click-extract UX on macOS. mtime-as-state removes a whole class of drift bugs | Switch to the `zip` crate if cross-platform users want Finder-native archives |
| Feature-flags presentation (Phase 8 kickoff) | **`[flags]` lands as a plain bool table** (`semantic_terminal`, `instant_connections`, `fleet_health`, `output_triggers`, `runbooks`, `automation`, `ai_assist`, `themes` — all default false); flag definitions (label, phase) live in frontend code; the Settings section renders them as **disabled toggles labeled "arrives in Phase N"** | §10 puts feature flags in the Phase 8 window, but a live toggle that controls nothing is dead UI (StatusBar precedent); disabled-with-phase-label is honest and the schema is ready for Phases 10–13 to flip their rows live | Hide the section entirely if the disabled rows read as clutter in review |
| Sync status channel (Phase 8 kickoff) | **One `sync:update` event carrying the fresh `GitSyncStatus`**, emitted after `git_sync_run` / `git_sync_set_remote` and recomputes — not per-state channels, not polling from both windows | Same single-consumer reasoning as `reach:update` / `forward:update`, plus it is what keeps the main-window footer live when the Settings window mutates sync state | Split channels if a per-row consumer ever appears |
| Homebrew tap naming (Phase 9 kickoff) | **`rpandox/homebrew-tap`** — install is `brew install --cask rpandox/tap/setu`, not §10's literal `pandox/tap/setu` | The GitHub account that owns setu is @rpandox; a `pandox` org doesn't exist and the tap must live where the releases live | Rename/move the tap if a pandox org ever materializes |
| v1.0.0 signing (Phase 9 kickoff) | **Ships unsigned (Tauri ad-hoc signature)** even though a Developer ID exists; first launch needs right-click → Open or `xattr -dr com.apple.quarantine` — documented in the cask caveats, README, and RELEASING.md. Codesign + notarize is the documented v1.0.1 follow-up | User decision at kickoff: ship v1.0.0 now, wire signing next release; RELEASING.md carries the full `codesign`/`notarytool`/`stapler` path so it's one manual run away | Sign v1.0.1 with the existing Developer ID per RELEASING.md |
| Updater (Phase 9 kickoff) | **tauri-plugin-updater deferred** — `brew upgrade --cask setu` plus GitHub Releases is the update path | The updater needs its own signing keypair and behaves best on a notarized app; neither exists for v1.0.0 | Add the updater when release signing lands |
| Cask artifact (Phase 9 kickoff) | **The cask installs a `.app.tar.gz`** from the GitHub release; a DMG is also attached for direct download | tar.gz avoids Gatekeeper's extra dmg-mount friction on an unsigned app and is what Homebrew unpacks natively; direct-download users still get the familiar DMG | Point the cask at the DMG once releases are notarized |
| Release build profile (Phase 9 kickoff) | **`[profile.release]`: fat LTO, `codegen-units = 1`, `strip = true`, `panic = "abort"`; opt-level stays 3** | Targets the < 25 MB acceptance bar; slower release builds are irrelevant to dev loops (`tauri dev` uses the dev profile). Terminal throughput keeps opt-level at 3 over `"s"` | Drop `panic = "abort"` if any dep ever needs unwinding; `strip = false` if crash symbolication is wanted |
| macOS floor (Phase 9 kickoff) | **`minimumSystemVersion = "12.0"`** in tauri.conf.json | README has required macOS 12+ since Phase 0; making the bundle enforce it turns a cryptic dyld crash on old systems into a clean "requires macOS 12" dialog | Raise only if a future dep demands it |
| Fresh-machine acceptance (Phase 9 kickoff) | **The "fresh-machine cask install" item is evidenced by an uninstall → `brew install --cask` → launch → connect cycle on the dev machine**, recorded as simulated | Only one Mac is available to this project; the cycle exercises the same cask, artifact, quarantine, and first-launch path — what it can't prove (no dev toolchain present) is noted honestly in the review | Re-run on a second Mac or fresh macOS VM when one exists |
| Perf-evidence hardware (Phase 9) | **The cold-launch bar is measured on the Intel dev Mac (i5-1038NG7): 1.29–1.32 s to page load** — the only machine available; the acceptance text says "on Apple silicon", where the same build can only be faster. The arm64 slice is verified by `lipo` but not executed | The §10 bar assumed the dev machine was Apple silicon; it isn't. An under-bar Intel number is a conservative pass, recorded as such rather than pretended | Re-measure on Apple silicon hardware when available |
| Signing follow-up (post-v1.0.0) | **Signing/notarization shelved indefinitely — there is no Apple Developer Program membership.** The v1.0.0-signing row's premise ("a Developer ID exists") was mistaken: no paid membership, so no certificate and no notarization at any price the project pays today. Releases stay ad-hoc-signed; README and RELEASING §7 document three quarantine outs, including `brew install --cask --no-quarantine`. RELEASING §8 keeps the corrected flow (notarize + staple the app **before** packaging DMG/tar.gz, spctl gate) ready but unrunnable | The program is $99/yr; there is no free path to Gatekeeper-blessed distribution, and pretending v1.0.1 would be signed was worse than saying it plainly | Join the Apple Developer Program, then run RELEASING §8 as written |
...
`flate2` + `gethostname` + `time` (Phase 8: snapshot gzip, commit hostname,
UTC timestamp formatting — §5 snapshot-format row).
